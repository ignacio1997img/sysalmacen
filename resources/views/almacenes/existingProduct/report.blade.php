...
@if(auth()->user()->hasPermission('browse_existingproducts'))
                ...
                <div class="panel panel-bordered">
                    <div class="panel-body" style="padding: 0px">
                        <div class="col-md-8" style="padding: 0px">
                            <h1 class="page-title">
                                <i class="voyager-calendar"></i> Producto en Existencia
                            </h1>
                        </div>
                        <div class="col-md-4" style="margin-top: 30px">
                            <form name="form_search" id="form-search" action="{{ route('existingproducts.list') }}" method="POST">
                                @csrf
...
@section('javascript')
    ...
    <script>
        ...
        $('#sucursal_id').on('change',function()
        {
            ...
            if(id>=1)
            {
                $.get('{{route('ajax-sucursal-subalmacen.all')}}/'+id, function(data){
                    // alert(data)
                    var html_type=    '<option disabled selected value="">-- Seleccione una gestión --</option>'
                        // html_type +=    '@if(Auth::user()->hasRole("admin") )<option value="TODO">Todos</option>@endif'
